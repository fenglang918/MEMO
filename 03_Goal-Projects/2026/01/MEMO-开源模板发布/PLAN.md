...
- [x] 把可选扩展层显式建模进模板：repo-local bridge / cloud-sync mount / Codex local plugin
...
- PRISM ingest prompt：`.agents/skills/prism/SKILL.md`
- 核心 skill：`.agents/skills/me-network-crm/SKILL.md`
- People cards + design doc + CLI：`05_Resources/network/`
- Me profile + signals：`04_Assets/profile/`
- 双语切换（langpacks + apply）：`06_Infra/i18n/`
- 示例清理：`06_Infra/cleanup_examples.py`
- Read-first bridge：`06_Infra/plugins/lark-bridge/`
- Cloud sync mount：`06_Infra/plugins/Google-Drive/`
- Codex local plugin example：`.agents/plugins/marketplace.json`、`plugins/memo-audio-transcribe/`
- 示例项目（TPCD）：`03_Goal-Projects/2026/01/`
