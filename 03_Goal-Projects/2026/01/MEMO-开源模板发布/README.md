...
- PRISM ingest prompt（可复制给任意 AI）：`.agents/skills/prism/SKILL.md`
- 核心 skill（维护人脉库端到端）：`.agents/skills/me-network-crm/SKILL.md`
- 双语切换（一键覆盖 README/skills/prompt）：`06_Infra/i18n/switch_language.py`
- 示例清理（从内置示例收束为更空骨架）：`06_Infra/cleanup_examples.py`
- 可选本地 bridge：`06_Infra/plugins/lark-bridge/`
- 可选云盘挂载 / 同步入口：`06_Infra/plugins/Google-Drive/`
- 可选本地插件市场与插件示例：`.agents/plugins/marketplace.json`、`plugins/memo-audio-transcribe/`
...
2) 交给 Agent：按 `.agents/skills/prism/SKILL.md` 输出 Facts/Inferences/Actions/Me-Info
3) 写入对象：
...
- 证明：模板不只是一组 Markdown 目录，也保留了可选 bridge / plugin / cloud-sync 扩展位点
